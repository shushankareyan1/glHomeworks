...
Shader::Shader(const std::string& fileName, bool firstShader) {
    ...
    m_shaders[2] = CreateShader(LoadShader(fileName + "1.vs"), GL_VERTEX_SHADER);
    m_shaders[3] = CreateShader(LoadShader(fileName + "1.fs"), GL_FRAGMENT_SHADER);
    ...
    glAttachShader(m_program[1], m_shaders[2]);
    glAttachShader(m_program[1], m_shaders[3]);

    glLinkProgram(m_program[1]);
    CheckShaderError(m_program[1], GL_LINK_STATUS, true, "error: link phase fails");

    glValidateProgram(m_program[1]);
    CheckShaderError(m_program[1], GL_VALIDATE_STATUS, true, "error: program is invalid");
...
Shader::~Shader() {
    for(unsigned int i = 0; i < NUM_SHADERS; i++) {
        ...
        glDetachShader(m_program[1], m_shaders[i]);
    }
